...
async function scheduleCall(students, counselors, activities, rooms) {
    ...
    const result = await scheduleAlgorithm(groups, activities.activities, rooms);
            ...
            for (var g = 0; g < groups.length; g++) {
                ...
                for (var b = 0; b < all_blocks.length; b++) {
                    ...
                    for (var attemp = 0; attemp < MAX_ATTEMPT; attemp++) {
                        ...
                        var time = Math.floor(Math.random() * (TIME - all_blocks[b].activity_id.duration + 1));
                        // Check schedule availability
                        var occupied = false;
                        for (var j = time; j < time + all_blocks[b].activity_id.duration; j++) {
                        ...
                        for (var r = 0; r < all_blocks[b].activity_id.rooms.length; r++) {
                            selected_room_id = all_blocks[b].activity_id.rooms[r];
                            for (var j = time; j < time + all_blocks[b].activity_id.duration; j++) {
                        ...
                        if (selected_room_id !== "") {
                            for (var j = time; j < time + all_blocks[b].activity_id.duration; j++) {
                                console.log("Inserting", all_blocks[b].activity_id.name, "at day", day, "time", time);
