...
Users can add an image of the floorplan (for visual reference of each room). They can then manually add each room. The users can also manually add the potential activities that could take place in the schedule. Users can edit, search, delete, or add rooms and activites.
Users can also delete a whole database (used when the camp session ends). The floorplan management feature is found on the floorplan page. This feature is so that we have accurate information when generating a schedule.
### Schedule generation
After inputting profiles, rooms, and activities, users can generate schedules based on the inputted information. This is done through visiting the schedule page and clicking the generate schedule page. The generate schedule will also include the generating of groups based on inputted infromation such as preferences, sex and age. The purpose of this feature is to simplify the mixing and matching to create an ideal schedule.
### Account system
...
## Instructions
### Logging in
1. The user lands at the login in page before they can access the app. They input a username and password to login into a pre-exising account that we give to them during handoff. This step needs to be completed for the user to be granted authorization.
### Inputting Information
 2. Once in the app on the profiles page, they can insert data relating to students and counselors in a csv located in deliverables/yres_scheduler/yres_scheduler_frontend/src/app/data/student_add_data.csv (there is an example csv for formatting). They don't need to input any IDs for students or counselors, because we generate them. All available data will be displayed in an interactive table that allows for editing and deleting of students or counselors. The user can toggle between students and counselors through a dropdown. They can also choose to search for student/counselor profiles and can manually add profiles.
